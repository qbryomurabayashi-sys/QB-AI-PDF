...
export const SettingsModal = ({ isOpen, onClose, settings, onSave, onBgmUpload, bgmName }) => {
    ...
    const [activeTab, setActiveTab] = useState('APIキー');
    
    if (!isOpen) return null;

    const tabs = ['APIキー', '音声', 'オーディオ & ビデオ'];
                ...
                <div className="w-64 bg-[#050505] border-r border-white/5 p-6 flex flex-col gap-1">
                    ...
                    {tabs.map((item) => (
                        <button 
                            ...
                            className={`text-left px-4 py-2.5 rounded-lg text-sm font-medium cursor-pointer transition-all ${activeTab === item ? 'bg-white/5 text-white' : 'text-white/40 hover:text-white hover:bg-white/5'}`}
                        >
                            {item}
                        </button>
                    ))}
